...
def main():
    ...
    for month, model in zip(months, models):
        model = pickle.load(open('logreg_model_1months.sav', 'rb'))
        y_pred_bin = model.predict(X).astype(int)
        y_pred_weighted = []
        for i in range(-n_plot, -1):
            y_pred_bin_ = y_pred_bin[:i]
            y_pred_weighted.append(np.dot(np.linspace(0,1,21) / sum(np.linspace(0, 1, n_lookback)), y_pred_bin_[-n_lookback:]))
        y_pred_weighted.append(np.dot(np.linspace(0, 1, n_lookback) / sum(np.linspace(0, 1, n_lookback)), y_pred_bin[-n_lookback:]))
